...
def closest_distance(x,y,optimal_waypoints,ac_x,ac_y):
    ...
    for i in range(0,opt_len):
        list.append((eucledian_distance(x,y,optimal_waypoints[i][0],optimal_waypoints[i][1]),optimal_waypoints[i][0],optimal_waypoints[i][1]))
...
def reward_function(params):
    ...
    dist = closest_distance(params['x'],params['y'],optimal_waypoints,waypoints[closest_waypoints[1]][0],waypoints[closest_waypoints[1]][1])
    if dist>=0:
        reward = 100/(1+ 2*dist)
    if params['steps'] > 0:
